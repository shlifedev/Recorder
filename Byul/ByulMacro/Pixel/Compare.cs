            ...
            using (Mat mat1 = src.ToMat())
            {
                using (Mat mat2 = cur.ToMat())
                {
                    using (Mat mat3 = new Mat())
                    {
                        ...
                        Console.WriteLine("ismatching :" + maxVal +"," + flag);
                    }
                }
            }
            return flag;
        }
        public static void Match(this Bitmap src, out Mat oResult, out Point oCenter, out Point oMaxLoc, Bitmap template, double accurity = 0.8f, bool grayscaleCheck = false)
        {
            ...
            _template.SaveImage("debug/tmp.png");

            if (grayscaleCheck)
            {
                Console.WriteLine("grayscale mode");
            ...
            Cv2.Threshold(_matchResult, _matchResult, threshhold , 1 , ThresholdTypes.Tozero);
           ...
           //결과 출력을 위해 복제 및 그리기
            ...
            if (maxVal >= threshhold)
            {
                ...
                oResult = result;
           //     result.SaveImage("debug/" + maxVal.ToString() + ".png");
             //   _matchResult.SaveImage("debug/" + maxVal.ToString() + "_grayscale.png");
            }
            else
            {
